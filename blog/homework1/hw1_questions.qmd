...
```{r}
kable(summary_table[1:3, ], digits = 4, caption = "Summary of the simulated A/B test sample")
```
...
```{r}
kable(summary_table[4:7, ], digits = 4, caption = "Estimated uncertainty around the treatment effect")
```
...
```{r}
kable(summary_table[8:9, ], digits = 4, caption = "Hypothesis test results")
```
...
```{r}
kable(reg_table, digits = 4, caption = "Regression version of the A/B test")
```
...
```{r}
kable(peek_table, digits = 4, caption = "Effect of repeated peeking under the null")
```
